...
class FocusScraping:
    ...
    def __init__(self, link):
        ...
        self.soup = bs(self.focus_html, 'lxml')
        self.link = link
        self.liste = []
    ...
    def show_html(self):
        return print(self.focus_html)

    def show_info(self):
        os.chdir("/media/pi/datadrive/databank/FOCUS-SCRAPING/links")
    ...
    def create_json(self): 
        ...
        try:
            with open(f'{datum}-Focus_JSON-{cc}.json', 'r') as fp:
    ...
    def search_links(self):
        ...
        os.chdir("/media/pi/datadrive/databank/FOCUS-SCRAPING/links")
        with open(f'{datum}-Focus_data-JSON-{cc}.json', 'w+') as fp:
    ...
    def get_links(self):
        ...
        for i in soup.find_all('span', class_="date psDate-"):
            i1 = i.text
            i2 = i1.replace("\n","")
            i3 = i2.strip()
        ...
        with open(f'{datum}-Focus_links-{cc}.txt', 'w') as file:
...
cc = 1
print(" >>>>>>>>>>>>>>>>>>>> START <<<<<<<<<<<<<<<<<<<<<")
link = 'https://www.focus.de/politik/schlagzeilen/'
while master <= max_days:
    os.chdir("/media/pi/datadrive/databank/FOCUS-SCRAPING/links")
    now = datetime.datetime.now()
    datum = now.strftime("%d-%m-%Y")
    print("------>", datum, "---", uhrzeit, "<-------")
    run = FocusScraping(link)
    run.show_html()
    run.show_info()
    run.create_json()
    run.find_links()
